...
export default function App() {
  ...
  const {
    ...
    `https://api.tvmaze.com/singlesearch/shows?q=${serie}&embed[]=episodes&embed[]=cast&embed[]=seasons`
  );
    ...
    <div className="">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-center bg-background w-full p-4 rounded-md">
          <h1 className="text-2xl font-bold uppercase bg-gradient bg-clip-text text-transparent ">
            Vos films et séries, en ULTRA HD !
          </h1>
        ...
        {showData && (
          <section className="w-full rounded-md shadow-sm shadow-accent">
            <div className="bg-background rounded-md">
              <h2 className="text-xl font-bold text-center bg-gradient bg-clip-text text-transparent p-4 w-full">
                {showData.name}
              </h2>
            </div>
                    ...
                    <div className="bottom-0 m-4">
                      ...
                      <div className="flex gap-4 pt-4">
                        {showData?.genres.map((genre, index) => (
                          <p key={index} className="bg-muted p-1 rounded-md">
                            {genre}
                          </p>
                        ))}
                      </div>
              ...
              {selectedInfo === "episodes" && (
                <div className="grid grid-cols-1sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {showData._embedded.episodes.map((film) => (
                    <div
                      key={film.id}
                      className="grid gap-4 md:flex-row border-b border-gray-200 bg-background rounded-md overflow-hidden"
                    >
                      <img
                        className="h-auto w-full object-cover md:w-full"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {showData._embedded.cast.map((castMember) => (
                    <div
                      ...
                      className="grid grid-cols-2 gap-4 md:flex-row border-b border-accent bg-background rounded-md overflow-hidden"
                    >
                      <img
                        className="h-40 w-40 object-cover md:w-full"
              ...
              {selectedInfo === "seasons" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {showData._embedded.seasons.map((season) => (
                    <div
                      key={season.id}
                      className="grid gap-4 md:flex-row border-b border-accent bg-background rounded-md overflow-hidden"
                    >
                      <img
                        className="h-auto w-full object-cover md:w-full"
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-sm">
                          {season.premiereDate} - {season.endDate || "Ongoing"}
                        </p>
                        <p className="text-sm text-secondary mt-2">
      ...
      <div className="w-full flex flex-col gap-5 py-2 items-center justify-center">
        <Accordion
          type="single"
          collapsible
          className="bg-background px-4 rounded-md w-full"
        >
          ...
          <div className="flex justify-center items-center gap-2">
            <a href="">
              <img
                ...
                className="bg-primary rounded-full p-1 fill-foreground"
              />
            </a>
            <a href="">
              <img
        ...
        <div className="w-full bg-primary flex justify-center items-center rounded-md">
          <a href="">
            <img src="/Logo.svg" alt="Logo du site de streaming Pacifico" />
          </a>
        </div>
      </div>
    </div>
